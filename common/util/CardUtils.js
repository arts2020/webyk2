...
const CardUtils = {
	...
	getCardTypeName: function(cardType, istw) {
		if (this.cardTypeMap[cardType]) {
			return this.cardTypeMap[cardType]
		}
		if (istw === true) {
			return this.cardTypeMap_tw[cardType]
		}
		return undefined;
	},
	bankcardList: [{
		bankName: "中国邮政储蓄银行",
		bankName_tw: "中國郵政儲蓄銀行",
		bankCode: "PSBC",
		patterns: [{
	...
	getBankNameByBankCode: function(bankcode) {
		for (var i = 0, len = this.bankcardList.length; i < len; i++) {
			var bankcard = this.bankcardList[i];
	...
	getBankInfoByCardNo: function(cardNo) {
		...
		if (cardNo.length > 10) {
			var val = cardNo.substring(0, 6)
			for (var i = 0, len = this.bankcardList.length; i < len; i++) {
				var bankcard = this.bankcardList[i];
				var patterns = bankcard.patterns;
				for (var j = 0, jLen = patterns.length; j < jLen; j++) {
					var pattern = patterns[j];
					if ((new RegExp(val)).test(pattern.reg)) {
						var info = {}; // extend(bankcard, pattern);
						info.cardType = pattern.cardType;
						info.bankName = bankcard.bankName;
						...
						// }
						vue.cclog("===info.bankName=", info.bankName);
						vue.cclog("===cardNo=", cardNo);
						return info;
					}
				}
			}
