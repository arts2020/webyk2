<template>
	<view @touchmove.stop.prevent="moveHandle">
		...
		<view v-show="show" :class="'uni-popup-' + position + ' ' + 'uni-popup-' + mode" class="uni-popup">
			<view class="content">
				<view class="close" v-if="isshowclose" :style="titleBgColor.length > 0 ? 'background: '+ titleBgColor +';' : ''">
					<view :style="'padding-left: 20px; color:#282828;font-size:18px'">{{title}}</view>
					<HsIcon class="hsclose" type="cha" size="25px" @click="clickClose" style="color: #282828;"></HsIcon>
				</view>
				<slot />
			</view>
		</view>
