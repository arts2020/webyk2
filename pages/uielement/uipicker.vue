    ...
    <view class="mpvue-picker">
        ...
        <view class="mpvue-picker-content " :class="{'mpvue-picker-view-show':showPicker}">
            ...
            <picker-view indicator-style="height: 40px;" class="mpvue-picker-view" :value="pickerValue" @change="pickerChange" v-if="mode==='selector' && pickerValueSingleArray.length > 0">
                <block>
                    <picker-view-column>
                        <view class="picker-item" :style="{fontSize:row_title}" v-for="(item,index) in pickerValueSingleArray" :key="index">{{item.label}}</view>
                    </picker-view-column>
                </block>
            </picker-view>
            <!-- 时间选择器 -->
...
<script>
    export default {
        data() {
            return {
				row_title:this.fontSize.row_title,
                pickerChangeValue: [],
                pickerValue: [],
        ...
        methods: {
            ...
            _initPickerVale() {
				...
				uni.cclog("==this.pickerValue==",this.mode)
            }
        }
    };
</script>
