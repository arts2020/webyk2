...
<script>
	...
	export default {
		...
		created() {
			this.util.EventUtils.addEventListenerCustom(this.dal.Account.evtForget, this.onForget);
		},
		
		destroyed() {
			this.util.EventUtils.removeEventCustom(this.dal.Account.evtForget, this.onForget);
		},
		data() {
			return {
				username: '',
		...
		methods: {
			...
			btnYzm:function(){
				...
				this.dal.Account.onGetCaptcha(username,this.entities.Metadata.SmsType.XXBG)
			},
			
			onForget:function(){
				uni.showModal({
					title: this.getLocalStr("tip_title"),
					content: "密码修改成功，请用新密码重新登录",
			...
			btnConfirm:function(){
				...
				this.util.UiUtils.showLoading("找回密码中...")
				this.dal.Account.onForget(username,password,useryzm)
			},
		},
	}
</script>
